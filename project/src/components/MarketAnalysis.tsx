...
export const MarketAnalysis: React.FC<MarketAnalysisProps> = ({
  marketData = [], // Default to empty array
  analysisMode,
  onModeChange,
  onScreenshotUpload,
  isLoading = false,
  ...
  const [showAllPairs, setShowAllPairs] = useState(false); // Default to collapsed (show less)
  ...
  // Show only Tier 1 pairs by default, or all pairs if showAllPairs is true
  const displayedData = showAllPairs ? safeMarketData : tier1Pairs;
          ...
          <div className="space-y-3">
            {isLoading ? (
              ...
              <>
                ...
                {showAllPairs && tier2Pairs.length > 0 && (
            ...
            <span>
              {analysisMode === 'api' 
                ? isLoading 
                  ...
                  ? 'Using fallback data' 
                  : showAllPairs 
                  ? `Live data: Tier 1 (${tier1Pairs.length}) + Tier 2 (${tier2Pairs.length}) pairs`
                  : `Live data: Tier 1 (${tier1Pairs.length}) pairs`
                : 'Screenshot mode'
              }
            </span>
          </div>
