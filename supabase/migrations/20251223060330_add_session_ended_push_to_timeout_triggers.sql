/*
  # Add session_ended Push Notifications to Timeout Triggers

  1. Changes
    - Update check_continuation_modal_timeout() to insert session_ended notification
    - Update force_close_stale_session() to insert session_ended notification
    - Ensures users get push notifications when sessions auto-close due to timeout

  2. Security
    - Functions use SECURITY DEFINER (already set)
    - Only authenticated users can call these functions
    - Notifications use proper user_id from session

  3. Context
    - When 15-min timeout triggers, user gets "Session Ended - Timeout" push
    - When 20-min safety net triggers, user gets "Session Ended - Safety Stop" push
    - When user force closes, user gets "Session Ended - User Stopped" push
*/
