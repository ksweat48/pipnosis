...
CREATE TABLE IF NOT EXISTS ai_skill_progression (
  ...
  skill_level_numeric integer NOT NULL DEFAULT 1,
  ...
  current_confidence_accuracy numeric(5,2) NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_ai_skill_progression_user ON ai_skill_progression(user_id);
CREATE INDEX IF NOT EXISTS idx_ai_skill_progression_level ON ai_skill_progression(user_id, skill_level_numeric);
...
CREATE INDEX IF NOT EXISTS idx_ai_learning_milestones_user ON ai_learning_milestones(user_id, achieved_at DESC);
...
CREATE TABLE IF NOT EXISTS ai_indicator_experiments (
  ...
  improvement_vs_baseline numeric(5,2) NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_ai_indicator_experiments_user ON ai_indicator_experiments(user_id, experiment_status);
CREATE INDEX IF NOT EXISTS idx_ai_indicator_experiments_name ON ai_indicator_experiments(user_id, indicator_name);
...
CREATE TABLE IF NOT EXISTS ai_indicator_effectiveness (
  ...
  signal_quality_score numeric(5,2) NOT NULL DEFAULT 0,

  is_currently_active boolean DEFAULT true,
  weight_in_decision numeric(5,2) DEFAULT 50.00,
...
CREATE INDEX IF NOT EXISTS idx_ai_indicator_effectiveness_user ON ai_indicator_effectiveness(user_id, symbol, timeframe);
CREATE INDEX IF NOT EXISTS idx_ai_indicator_effectiveness_active ON ai_indicator_effectiveness(user_id, is_currently_active);
CREATE INDEX IF NOT EXISTS idx_ai_indicator_effectiveness_quality ON ai_indicator_effectiveness(user_id, signal_quality_score DESC);
...
CREATE INDEX IF NOT EXISTS idx_ai_indicator_usage_history_user ON ai_indicator_usage_history(user_id, action_date DESC);
