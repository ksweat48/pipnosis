/*
  ...
  1. Problem
    - The ORDER BY clause uses "created_at" without table qualification
    - Both realtime_prices and goal_session_trades tables have a created_at column
    - PostgreSQL error: "column reference 'created_at' is ambiguous"
    - Error code: 42702

  2. Solution
    - Fully qualify the column as realtime_prices.created_at
    - Ensures PostgreSQL knows which table's created_at to use

  3. Changes
    - Update admin_get_all_users function
    - Change ORDER BY created_at to ORDER BY realtime_prices.created_at
*/

-- Drop existing function
DROP FUNCTION IF EXISTS admin_get_all_users(text, integer);

-- Recreate with fully qualified column reference
      ...
      (SELECT jsonb_agg(
       ...
       FROM (
         ...
         LEFT JOIN LATERAL (
           ...
           ORDER BY symbol, realtime_prices.created_at DESC  -- FIX: Fully qualified column
