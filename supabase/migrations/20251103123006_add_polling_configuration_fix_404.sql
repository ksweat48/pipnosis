...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'polling_rate_limits' AND policyname = 'Users can view own rate limit data'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'polling_rate_limits' AND policyname = 'Users can insert own rate limit data'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'symbol_volatility_tracking' AND policyname = 'All authenticated users can view volatility data'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'symbol_volatility_tracking' AND policyname = 'Only authenticated users can update volatility data'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'symbol_volatility_tracking' AND policyname = 'Only authenticated users can modify volatility data'
  ) THEN
...
BEGIN
  ...
  -- Build array with new price and last 19 samples
  IF jsonb_array_length(v_samples) >= 20 THEN
    v_samples := (
      SELECT jsonb_agg(elem)
      FROM (
        SELECT elem
        FROM jsonb_array_elements(v_samples) elem
        LIMIT 19
      ) sub
    ) || to_jsonb(p_current_price);
  ELSE
    v_samples := v_samples || to_jsonb(p_current_price);
  END IF;
  ...
  IF array_length(v_prices, 1) IS NOT NULL AND array_length(v_prices, 1) > 0 THEN
    -- Calculate mean
    ...
    v_std_dev := sqrt(COALESCE(v_variance, 0));
  ...
  DO UPDATE SET
    volatility_score = COALESCE(v_volatility_score, 0),
    price_std_dev = COALESCE(v_std_dev, 0),
    last_updated = now(),
