/*
  # Fix check_and_close_positions_on_price_update trigger - remove invalid NEW.price reference

  1. Problem
    - The trigger function references `NEW.price` which does not exist on the `realtime_prices` table
    - The table has columns: bid, ask, mid (not "price")
    - Every INSERT into realtime_prices fails with: "record new has no field price"
    - This breaks the entire price pipeline — no prices flow into the database

  2. Fix
    - Replace `COALESCE(NEW.bid, NEW.price)` with `NEW.bid`
    - Replace `COALESCE(NEW.ask, NEW.price)` with `NEW.ask`
    - Replace `NEW.price` (used as current_price) with `NEW.mid`
    - bid and ask are always populated by the hybrid-price-collector, no fallback needed

  3. Impact
    - Restores price inserts to realtime_prices table
    - Re-enables the entire trading data pipeline
    - SL/TP trigger monitoring resumes working
*/
