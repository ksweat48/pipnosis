...
BEGIN
  ...
  FOR v_trade IN
    ...
    WHERE session_id = p_session_id
  LOOP
    -- Create trade analysis record (FIXED: Use flow_v2_confidence instead of confidence_score)
    INSERT INTO ai_trade_analysis (
      ...
      v_trade.pnl,
      v_trade.entry_time,
      COALESCE(v_trade.flow_v2_confidence, 75), -- FIXED: Changed from confidence_score
      '{}'::jsonb,
  ...
  -- Update AI performance evolution
