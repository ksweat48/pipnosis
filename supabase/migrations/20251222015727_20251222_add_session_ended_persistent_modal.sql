/*
  # Add Session Ended Persistent Modal System

  1. Problem Statement
    Users who are away from the app when their session closes (due to timeout, 
    1-minute countdown expiring, etc.) never see any feedback about what happened.
    They return to find the session closed with no explanation.

  2. Solution
    - Add 'session_ended' modal type to pending_user_modals
    - When sessions close via timeout (check_continuation_modal_timeout), 
      create a persistent modal that users will see when they return
    - Store session summary data (duration, reason, trades count)

  3. Changes
    - Add 'session_ended' to valid modal types constraint
    - Update check_continuation_modal_timeout to create session_ended modal
    - Update force_close_stale_session to create session_ended modal
    - Create helper function create_session_ended_modal
...
-- ============================================================================
-- STEP 1: Add 'session_ended' to valid modal types
-- ============================================================================
...
ALTER TABLE pending_user_modals
  ...
  CHECK (modal_type IN ('trade_closed', 'goal_achieved', 'session_update', 'continuation', 'session_ended'));

COMMENT ON CONSTRAINT valid_modal_type ON pending_user_modals IS
  'Valid modal types: trade_closed, goal_achieved, session_update, continuation, session_ended';

-- ============================================================================
-- STEP 2: Create helper function to create session_ended modal
-- ============================================================================
...
-- ============================================================================
-- STEP 3: Update check_continuation_modal_timeout to create session_ended modal
-- ============================================================================
...
COMMENT ON FUNCTION check_continuation_modal_timeout IS
  'Checks if continuation modal has timed out (1 minute) and auto-closes session. Creates session_ended modal for user feedback. Also includes 20-minute safety net for stuck sessions.';

-- ============================================================================
-- STEP 4: Update force_close_stale_session to create session_ended modal
-- ============================================================================
...
COMMENT ON FUNCTION force_close_stale_session IS
  'Allows users to force-close their own stale sessions. Creates session_ended modal for feedback.';

-- ============================================================================
-- STEP 5: Drop and recreate handle_continuation_response with updated logic
-- ============================================================================

DROP FUNCTION IF EXISTS handle_continuation_response(uuid, boolean);
...
COMMENT ON FUNCTION handle_continuation_response IS
  'Handles user response to continuation modal. If continuing, resets scan timer. If stopping, closes session.';
