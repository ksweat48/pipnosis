/*
  # CCIP-2026-0511S: Atomic session termination on trade closure

  ## Summary
  Modifies `close_goal_session_trade` RPC to atomically terminate the parent
  goal_session when all execution channels are empty after the trade closes.

  This is the SSOT fix for session-stuck-in-scanning bugs. Regardless of which
  client path invokes the close (coordinator, position-service, live-engine,
  realtime), the RPC guarantees the session state matches reality.

  ## Problem
  When a trade closes via a path that bypasses
  tradeClosureCoordinator.updateSessionAfterClose(), the goal_session.status
  never transitions out of 'scanning' even though in-memory governance has
  halted scanning. The UI remains stuck showing "Scanning" forever.

  ## Contract
  After the trade row is updated and the balance is applied, the RPC:
    1. Counts remaining open/pending trades for this session
    2. Counts remaining monitoring entry_intents for this session
    3. If ALL channels are empty AND the session is currently in a live state
       (active/scanning/in_trade), transitions the session to a terminal state:
         - system_stopped: stop_loss, take_profit, take_profit_1, take_profit_2,
                          timeout, weekend_protection, session_ended, risk_limit,
                          trailing_stop, holiday_closure, market_closed, goal_expired
         - user_stopped:   manual, force_closed
         - goal_achieved is NEVER set here — that remains the GoalAchievementCoordinator's
           authority.
    4. Sets completed_at to the trade's closed_at.

  Both 'system_stopped' and 'user_stopped' already resolve to logical 'stopped'
  via DB_STATUS_REVERSE on the client — no UI changes required.

  ## Safety
    - Uses optimistic status filter so we never overwrite goal_achieved.
    - Wrapped in BEGIN/EXCEPTION so termination failure does NOT fail the close.
...
DECLARE
  ...
  v_open_trades_remaining integer;
  v_pending_trades_remaining integer;
  v_active_intents_remaining integer;
  v_session_terminal_status text;
  v_session_rows_updated integer;
BEGIN
  ...
  -- CCIP-2026-0511S: Atomic session termination when all execution channels empty
  -- Non-fatal: a failure here must NOT fail the trade close.
  IF v_trade.goal_session_id IS NOT NULL THEN
    BEGIN
      SELECT COUNT(*) INTO v_open_trades_remaining
      FROM goal_session_trades
      WHERE goal_session_id = v_trade.goal_session_id
        AND status = 'open'
        AND id <> p_trade_id;

      SELECT COUNT(*) INTO v_pending_trades_remaining
      FROM goal_session_trades
      WHERE goal_session_id = v_trade.goal_session_id
        AND status = 'pending'
        AND id <> p_trade_id;

      -- entry_intents uses session_id (not goal_session_id) and 'monitoring' status
      SELECT COUNT(*) INTO v_active_intents_remaining
      FROM entry_intents
      WHERE session_id = v_trade.goal_session_id
        AND status = 'monitoring';

      IF v_open_trades_remaining = 0
         AND v_pending_trades_remaining = 0
         AND v_active_intents_remaining = 0 THEN

        IF p_close_reason IN ('manual', 'force_closed') THEN
          v_session_terminal_status := 'user_stopped';
        ELSE
          v_session_terminal_status := 'system_stopped';
        END IF;

        -- Only transition live sessions. Never overwrite goal_achieved or
        -- an already-terminal status.
        UPDATE goal_sessions
        SET
          status = v_session_terminal_status,
          completed_at = v_actual_closed_at,
          updated_at = now()
        WHERE id = v_trade.goal_session_id
          AND status IN ('active', 'scanning', 'in_trade', 'paused');

        GET DIAGNOSTICS v_session_rows_updated = ROW_COUNT;

        IF v_session_rows_updated > 0 THEN
          RAISE LOG '[close_goal_session_trade] CCIP-2026-0511S: Session % terminated (%) after all channels emptied',
            v_trade.goal_session_id, v_session_terminal_status;
        END IF;
      END IF;
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING '[close_goal_session_trade] CCIP-2026-0511S session termination non-fatal error: %', SQLERRM;
    END;
  END IF;
