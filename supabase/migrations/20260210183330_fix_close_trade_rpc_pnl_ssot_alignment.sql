/*
  # Fix close_goal_session_trade RPC P&L Calculation - SSOT Alignment

  ## Root Cause
  The close_goal_session_trade RPC (created in migration 20260209182907) introduced
  P&L formulas that are INCONSISTENT with the JavaScript SSOT in currencyHelpers.ts.

  For XAUUSD (Gold):
  - RPC used: (price_diff / 0.01) * (lot_size * 100) = 100x TOO LARGE
  - SSOT defines: pipValue=1.0, dollarPerPipPerLot=100 -> price_diff * lot_size * 100

  This caused trade d2c76fb8 (XAUUSD BUY 0.23 lots) to show -$22,977 instead of -$229.77.

  ## Affected Symbols & Corrections (aligned to currencyHelpers.ts)
  - XAUUSD: WAS (diff/0.01)*(lot*100) -> NOW diff*lot*100 [pipValue=1.0, $100/pip/lot]
  - XAGUSD: WAS (diff/0.001)*(lot*50) -> NOW diff*lot*5.0 [pipValue=1.0, $5/pip/lot]
  - Indices: WAS diff*lot -> NOW diff*lot*100 [pipValue=1.0, $100/pip/lot]
  - JPY: WAS (diff/0.01)*(lot*1000) -> NOW (diff/0.01)*(lot*10) [pipValue=0.01, $10/pip/lot]
  - BTC/ETH: UNCHANGED diff*lot [pipValue=1.0, $1/pip/lot] (already correct)
  - Forex: UNCHANGED (diff/0.0001)*(lot*10) [pipValue=0.0001, $10/pip/lot] (already correct)

  ## Security
  - SECURITY DEFINER with restricted search_path
  - Same RLS and ownership checks as before
  - P&L sanity check added: warns if |P&L| > $50,000

  ## CCIP Governance
  - Change reason: SSOT violation - RPC must match currencyHelpers.ts
  - Affected: close_goal_session_trade RPC function
  - Risk: LOW (formula correction, no schema changes)
*/
...
BEGIN
  ...
  /*
    P&L FORMULAS - ALIGNED TO currencyHelpers.ts SSOT
    Formula: (price_diff / pipValue) * (lot_size * dollarPerPipPerLot)
    For direction='sell', result is negated.
  */

  IF v_trade.symbol LIKE '%JPY%' THEN
    -- JPY pairs: pipValue=0.01, dollarPerPipPerLot=10
  ...
  ELSIF v_trade.symbol IN ('US30', 'NAS100', 'SPX500', 'DJI', 'NDX')
    ...
    -- Indices: pipValue=1.0, dollarPerPipPerLot=100
    v_pip_value := v_price_diff * v_effective_lot * 100;

  ELSIF v_trade.symbol LIKE '%XAU%' OR v_trade.symbol LIKE '%GOLD%' THEN
    -- XAUUSD (Gold): pipValue=1.0, dollarPerPipPerLot=100
    -- CRITICAL FIX: Was (diff/0.01)*(lot*100) = 100x too large
    v_pip_value := v_price_diff * v_effective_lot * 100;

  ELSIF v_trade.symbol LIKE '%XAG%' OR v_trade.symbol LIKE '%SILVER%' THEN
    -- XAGUSD (Silver): pipValue=1.0, dollarPerPipPerLot=5.0
  ...
  ELSIF v_trade.symbol LIKE '%BTC%' OR v_trade.symbol LIKE '%ETH%' OR v_trade.symbol LIKE '%CRYPTO%' THEN
    -- Crypto: pipValue=1.0, dollarPerPipPerLot=1.0
    v_pip_value := v_price_diff * v_effective_lot;

  ELSE
    -- Default forex: pipValue=0.0001, dollarPerPipPerLot=10
