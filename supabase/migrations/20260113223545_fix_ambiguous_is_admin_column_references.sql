/*
  # Fix Ambiguous is_admin Column References

  ## Issue:
  PostgreSQL error: "column reference 'is_admin' is ambiguous"

  ## Root Cause:
  When a function has `is_admin` in its RETURNS TABLE clause AND references
  `is_admin` in the WHERE clause without a table qualifier, PostgreSQL cannot
  determine which column is being referenced.

  ## Fix:
  Explicitly qualify all `is_admin` references with the table name `user_profiles.is_admin`
*/
...
BEGIN
  ...
  -- ✅ FIX: Explicitly qualify is_admin with table name
  IF NOT EXISTS (
...
BEGIN
  ...
  -- ✅ FIX: Explicitly qualify is_admin with table name
  IF NOT EXISTS (
    SELECT 1 FROM user_profiles WHERE id = calling_user_id AND user_profiles.is_admin = true
  ) THEN
    RAISE EXCEPTION 'Admin access required';
  END IF;
...
DO $$
BEGIN
  RAISE NOTICE '✓ Ambiguous is_admin column references fixed';
  RAISE NOTICE '  - admin_get_all_users_paginated: user_profiles.is_admin qualified';
  RAISE NOTICE '  - force_close_stale_scanning_sessions: user_profiles.is_admin qualified';
END $$;