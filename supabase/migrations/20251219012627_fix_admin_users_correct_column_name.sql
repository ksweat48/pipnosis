/*
  # Fix Admin Get All Users - Use Correct Column Name

  ## Problem
  - Function references gs.scanning_session_started_at which doesn't exist
  - The goal_sessions table uses "start_time" not "scanning_session_started_at"

  ## Solution
  - Update scanning duration calculation to use gs.start_time
  - This shows how long the session has been running since it started
*/
...
BEGIN
  ...
  SELECT
    ...
    -- Calculate scanning duration in minutes using start_time
    (
      SELECT EXTRACT(EPOCH FROM (NOW() - gs.start_time))/60
