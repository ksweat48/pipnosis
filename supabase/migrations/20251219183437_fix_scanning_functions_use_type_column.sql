/*
  # Fix Scanning Functions - Use Correct Column Name 'type'

  ## Problem
  The `trigger_scanning_cooldown` and `trigger_scanning_lockdown` functions are trying
  to INSERT into a column called `notification_type`, but the column was renamed to `type`
  in migration 20251215050454. This causes 400 errors when trying to insert notifications.

  ## Solution
  Update both functions to use the correct column name: `type` instead of `notification_type`
...
BEGIN
  ...
  -- Create notification in goal_notifications table with correct column name 'type'
...
BEGIN
  ...
  -- Create urgent notification in goal_notifications table with correct column name 'type'
  INSERT INTO goal_notifications (
    goal_session_id,
    user_id,
    type,
