/*
  # Fix: admin_get_all_users_paginated — is_admin Ambiguity

  ## Problem
  PostgreSQL PL/pgSQL scopes RETURNS TABLE column names as local variables within
  the function body. The output column "is_admin boolean" in RETURNS TABLE conflicts
  with the table column "up.is_admin" from user_profiles, producing:

    ERROR 42702: column reference "is_admin" is ambiguous
    DETAIL: It could refer to either a PL/pgSQL variable or a table column.

  This caused every call to admin_get_all_users_paginated to return HTTP 400,
  breaking the Admin Dashboard user list, credit updates, and force-refresh.

  ## SSOT / CCIP Compliance
  - Single authoritative fix: rename only the output column in RETURNS TABLE
    from "is_admin" to "user_is_admin". The SELECT body continues to reference
    "up.is_admin" which is now fully unambiguous.
  - The admin-user-service.ts normalization layer (line 211) already handles
    both output names: row.user_is_admin ?? row.is_admin ?? false
    No frontend changes required — the existing fallback path activates.
  - No data loss. No destructive DDL. DROP OR REPLACE is used.
  - No other functions, tables, or RLS policies are affected.

  ## Changes
  1. DROP the existing overloaded function signature (required before OR REPLACE
     when the return type shape changes).
  2. CREATE OR REPLACE with "user_is_admin" as the output column name.

  ## Governance
  - Root cause fixed at the database layer (SSOT for admin user data).
  - No business logic altered — only the column alias in RETURNS TABLE.
  - Post-deploy verification: call the RPC, confirm no 42702 error.
*/

-- Step 1: Drop the existing function (return type change requires explicit drop)
DROP FUNCTION IF EXISTS public.admin_get_all_users_paginated(text, integer, integer);

-- Step 2: Recreate with unambiguous output column name
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = calling_user_id AND user_profiles.is_admin = true
  ) THEN
