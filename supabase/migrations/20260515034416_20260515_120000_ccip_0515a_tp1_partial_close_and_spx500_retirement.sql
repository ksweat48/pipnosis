/*
  # CCIP-2026-0515A — TP1 Partial Close (Profit Preservation) + SPX500 Trigger Cleanup

  ## Summary
  Implements per-trade TP1 profit preservation by atomically reducing lot_size
  by `partial_close_pct` (default 0.5) when TP1 is hit on non-scalp trades, and
  removes residual SPX500 references from the position monitoring trigger.
  The `goal_session_trades_no_retired_symbols_chk` constraint already enforces
  SPX500 retirement at the row level — this migration only cleans the trigger
  body and adds an `original_lot_size` audit column.

  ## Changes
  1. New column: `goal_session_trades.original_lot_size` (numeric) — captures
     the pre-TP1 lot for audit and PnL math.
  2. Backfill: copies current `lot_size` (or `position_size`) into
     `original_lot_size` for all non-SPX500 rows where it is null.
  3. Trigger rewrite: `check_and_close_positions_on_price_update` now
     - Reduces `lot_size` to `lot_size * (1 - partial_close_pct)` at TP1 (non-scalp)
     - Sets `original_lot_size` on first TP1 hit
     - Computes `tp1_pnl` against the closed portion only
     - Removes SPX500 from both fallback ATR maps
     - Skips partial close if remaining lot would fall below 0.01

  ## Safety
  - `original_lot_size` backfill is scoped `symbol <> 'SPX500'` so the
    pre-existing retired-symbols CHECK is not re-validated against legacy rows.
  - Trigger uses optimistic update on `tp1_hit = false` (idempotent).
  - Partial close gated by `lot_size * (1 - partial_close_pct) >= 0.01`.
*/

-- 1. Add original_lot_size audit column (if not present)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'goal_session_trades' AND column_name = 'original_lot_size'
  ) THEN
    ALTER TABLE goal_session_trades ADD COLUMN original_lot_size numeric;
  END IF;
END $$;

-- 2. Backfill original_lot_size for non-SPX500 rows only
UPDATE goal_session_trades
SET original_lot_size = COALESCE(lot_size, position_size)
WHERE original_lot_size IS NULL
  AND symbol <> 'SPX500';

-- 3. Rewrite TP1 trigger with partial close logic + SPX500 cleanup
...
BEGIN
  ...
  LOOP
    ...
    THEN
      IF (v_position.direction = 'buy'  AND NEW.price >= v_position.tp1_price)
      OR (v_position.direction = 'sell' AND NEW.price <= v_position.tp1_price)
      THEN
    ...
    IF v_position.stop_loss IS NOT NULL THEN
      IF (v_position.direction = 'buy'  AND NEW.price <= v_position.stop_loss)
      OR (v_position.direction = 'sell' AND NEW.price >= v_position.stop_loss)
      THEN
        v_should_close := true;
    ...
    IF NOT v_should_close THEN
      IF v_position.tp2_price IS NOT NULL AND v_position.tp1_hit = true THEN
        IF (v_position.direction = 'buy'  AND NEW.price >= v_position.tp2_price)
        OR (v_position.direction = 'sell' AND NEW.price <= v_position.tp2_price)
        THEN
          v_should_close := true;
      ...
      ELSIF v_position.take_profit IS NOT NULL THEN
        IF (v_position.direction = 'buy'  AND NEW.price >= v_position.take_profit)
        OR (v_position.direction = 'sell' AND NEW.price <= v_position.take_profit)
        THEN
          v_should_close := true;
    ...
    IF NOT v_should_close AND v_position.stop_loss IS NULL THEN
      ...
      IF (v_position.direction = 'buy'  AND NEW.price <= v_position.entry_price - (v_atr * 3))
      OR (v_position.direction = 'sell' AND NEW.price >= v_position.entry_price + (v_atr * 3))
      THEN
        v_should_close := true;
...
COMMENT ON FUNCTION check_and_close_positions_on_price_update() IS
  'CCIP-2026-0515A: TP1 partial close (default 50%) reduces lot_size atomically, sets BE stop, and tp2_pnl tracks runner profit. SPX500 removed from fallback ATR map.';
