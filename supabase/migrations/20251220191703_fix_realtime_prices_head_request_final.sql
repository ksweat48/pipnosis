/*
  # Fix Realtime Prices HEAD Request 500 Error - Final Solution

  ## Problem
  Background aggregator gets 500 error when doing HEAD requests to count realtime_prices records.
  This happens because:
  1. HEAD requests use anon/authenticated client
  2. Multiple conflicting RLS policies exist from previous fix attempts
  3. Market being closed makes issue more visible (no new data flowing)

  ## Root Cause
  - Too many overlapping policies from migration history
  - No clear SELECT policy for authenticated users
  - Validation trigger edge cases not fully handled
  ...
  - Allows frontend to query price data (needed for charts)
