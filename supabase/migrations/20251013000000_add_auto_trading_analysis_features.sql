/*
  ...
  1. Schema Changes
    - Add `analysis_view_mode` column to `chart_preferences` table
      - Stores user preference for analysis view (technical or autotrading)
      - Default: 'technical'

    - Create `auto_trading_status` table
      - Tracks real-time auto-trading scanning status
      - Stores which symbols are being monitored
      - Records current phase evaluations

    - Create `auto_trading_scan_history` table
      - Historical record of symbol scans
      - Phase validation results for each scan
  ...
  3. Important Notes
    - Global analysis view toggle affects all charts for the user
    - Scan history helps track performance and debugging
    - Status table enables real-time UI updates
*/

-- Add analysis view mode to chart preferences
DO $$
BEGIN
  IF NOT EXISTS (
