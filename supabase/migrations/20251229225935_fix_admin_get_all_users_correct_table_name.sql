/*
  # Fix admin_get_all_users - Use Correct Table Name

  ## Problem
  - Function references non-existent `token_balances` table
  - Actual table name is `user_token_balance`

  ## Solution
  - Update function to use `user_token_balance` table
  - Fix join condition to match correct column names

  ## Changes
  - Replace `token_balances` with `user_token_balance`
  - Update join: `tb.user_id` to `utb.user_id`
  - Update select: `tb.balance` to `utb.balance`
*/
