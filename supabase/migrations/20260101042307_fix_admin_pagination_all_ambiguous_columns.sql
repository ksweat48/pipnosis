/*
  # Fix All Ambiguous Column References in Admin Pagination

  ## Problem
  Multiple ambiguous column references in admin_get_all_users_paginated:
  - created_at appears in multiple tables (user_profiles, goal_sessions, goal_session_trades)
  - updated_at appears in goal_sessions
  - All column references in LATERAL joins need explicit table qualification

  ## Solution
  Explicitly qualify ALL column references in LATERAL joins to prevent ambiguity.

  ## Changes
  1. Qualify all created_at references in LATERAL joins
  2. Qualify all updated_at references
  3. Qualify all other potentially ambiguous columns
*/

-- Fix the function with ALL columns explicitly qualified
