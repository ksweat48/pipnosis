/*
  # Prioritize Live Users in Admin Dashboard

  ## Problem
  Admin dashboard currently sorts users by `last_activity DESC`, which means:
  - Users with open trades may be buried in the list
  - Users actively scanning for trades aren't prioritized
  - Hard for admins to see who's LIVE on the platform right now

  ## Solution
  Implement three-tier sorting priority:
  1. **Tier 1 (Top)**: Users with open trades → actively trading RIGHT NOW
  2. **Tier 2 (Middle)**: Users currently scanning → actively looking for trades
  3. **Tier 3 (Bottom)**: All other users sorted by last activity

  ## Changes
  - Recreate `admin_get_all_users` function with new ORDER BY clause
  - Use CASE expressions to create priority flags
  - Within each tier, maintain chronological sorting (most recent first)

  ## Impact
  - Live traders always visible at the top
  - Scanning users always in second tier
  - Admins can immediately see platform activity
  - Easier monitoring and support for active users
*/

-- Drop existing function
DROP FUNCTION IF EXISTS admin_get_all_users(text, integer);

-- Recreate with LIVE-FIRST sorting
