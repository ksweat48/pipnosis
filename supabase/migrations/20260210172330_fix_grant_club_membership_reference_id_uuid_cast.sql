/*
  # Fix grant_club_membership UUID type mismatch

  1. Bug
    - `club_token_ledger.reference_id` is typed `uuid`
    - The function was casting UUID values to TEXT with `::TEXT` before inserting
    - PostgreSQL rejected this with error 42804 on every Stripe webhook call

  2. Fix
    - Remove `::TEXT` cast from all three INSERT statements into `club_token_ledger`
    - `v_membership_id` (UUID) and `v_referral.id` (UUID) are inserted directly

  3. Affected Flow
    - Stripe webhook -> stripe-webhook.ts -> grant_club_membership RPC
    - Club membership purchases were failing silently
*/
