/*
  # Create Server-Side Club Membership Grant RPC

  1. New Functions
    - `grant_club_membership(p_user_id, p_package_id, p_stripe_session_id, p_stripe_payment_intent_id, p_amount_paid_usd)`
      Atomically grants a Club membership:
      - Creates membership record
      - Awards initial token allocation
      - Locks required token balance
      - Completes any pending referral with rewards
      - Creates system notification
    
  2. Security
    - SECURITY DEFINER for atomic cross-table writes
    - Called by Stripe webhook (service role) only
    - Validates package exists and is active
    - Prevents duplicate memberships for same user

  3. Important Notes
    - This is the SSOT for server-side membership provisioning
    - The frontend `grantMembership()` method delegates to this RPC
    - All token movements are logged to `club_token_ledger`
*/
...
CREATE OR REPLACE FUNCTION grant_club_membership(
  ...
  p_amount_paid_usd NUMERIC(10,2)
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- Award referral PIP bonus to referrer
    UPDATE club_token_balances
  ...
  -- 10. Create system notification
  INSERT INTO goal_notifications (
    user_id, type, priority, title, message, metadata
  ) VALUES (
    p_user_id, 'system', 'high',
