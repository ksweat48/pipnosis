/*
  # Fix Scanning Functions - Correct Notifications Table Name

  ## Problem
  The `trigger_scanning_cooldown` and `trigger_scanning_lockdown` functions are trying
  to INSERT into a table called `notifications`, but the actual table name is
  `goal_notifications`. This causes 400 errors when `can_scan_now` RPC is called.

  ## Solution
  Update both functions to use the correct table name: `goal_notifications`

  ## Changes
  1. Recreate `trigger_scanning_cooldown` with correct table name
  2. Recreate `trigger_scanning_lockdown` with correct table name
  3. Update column names to match goal_notifications schema
*/
...
BEGIN
  ...
  -- Create notification in goal_notifications table
...
BEGIN
  ...
  -- Create urgent notification in goal_notifications table
  INSERT INTO goal_notifications (
    goal_session_id,
    user_id,
    notification_type,
