/*
  ...
  ## Problem
  PostgreSQL error 42702: column reference "is_admin" is ambiguous
  - Could refer to either a PL/pgSQL variable or a table column

  ## Solution
  - Fully qualify all column references in the RETURN QUERY statement
  - Explicitly use table aliases for all columns to avoid ambiguity
  - Rename local variables if needed to avoid conflicts

  ## Changes
  - Drop and recreate admin_get_all_users with proper column qualification
*/
...
BEGIN
  ...
  WITH user_stats AS (
    SELECT
      ...
      tb.balance as user_credit_balance,
