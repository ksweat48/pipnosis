/*
  # Fix Session Closure RPC - Use Correct Table Name
  
  ## Problem Statement
  - RPC `atomic_close_goal_session` references non-existent `trades` table
  - Actual table name is `trade_records`
  - Results in "relation trades does not exist" error during session closure
  
  ## Root Cause
  - Schema mismatch: Function written for legacy table name
  - SSOT violation: Table reference incorrect, causing closure to fail
  
  ## Changes
  - Update RPC to query `trade_records` instead of `trades`
  - Update RPC to query `goal_session_trades` for goal-linked trades (SSOT aligned)
  - Preserve all other logic, error handling, and governance tracking
...
BEGIN
  ...
  BEGIN
    ...
    -- Step 5: Close open trades (use goal_session_trades view for SSOT compliance)
