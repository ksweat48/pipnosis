/*
  # Fix Admin Functions Column References

  1. Changes
    - Fix `admin_get_all_users` to use `up.id` instead of `up.user_id`
    - Fix all admin functions to use correct column name `id` from user_profiles table

  2. Issue
    - Functions were referencing `up.user_id` but user_profiles table uses `id` as primary key
*/

-- Function: Get all users with search capability (FIXED)
...
BEGIN
  -- FIXED: Changed up.user_id to up.id
  ...
  SELECT
    ...
    COALESCE(
      (SELECT COUNT(*) FROM trade_history th WHERE th.user_id = up.id),
      0
    ...
    GREATEST(
      up.created_at,
      COALESCE((SELECT MAX(closed_at) FROM trade_history th WHERE th.user_id = up.id), up.created_at),
...
-- Function: Clear stuck goal session (FIXED)
...
-- Function: Add credits to user (FIXED)
...
-- Function: Recalculate user balance (FIXED)
...
-- Function: Get detailed user information (FIXED)
