/*
  Quick Fix: AI Prediction System Tables

  If you're seeing an error about 'ai_pair_predictions' table not found:

  1. Copy this entire SQL script
  2. Go to your Supabase Dashboard: https://supabase.com/dashboard
  3. Select your project
  4. Navigate to SQL Editor (left sidebar)
  5. Paste this script into a new query
  6. Click "Run" to execute
  7. After execution, go to Settings > API
  8. Click the "Refresh" button to update the schema cache
  9. Return to your application and try again

  This will create the required tables for the AI prediction system.
*/
...
DROP POLICY IF EXISTS "Users can view own predictions" ON ai_pair_predictions;
DROP POLICY IF EXISTS "Users can insert own predictions" ON ai_pair_predictions;
DROP POLICY IF EXISTS "Users can update own predictions" ON ai_pair_predictions;
DROP POLICY IF EXISTS "Users can delete own predictions" ON ai_pair_predictions;

DROP POLICY IF EXISTS "Users can view own analysis snapshots" ON ai_pair_analysis_snapshots;
DROP POLICY IF EXISTS "Users can insert own analysis snapshots" ON ai_pair_analysis_snapshots;
DROP POLICY IF EXISTS "Users can update own analysis snapshots" ON ai_pair_analysis_snapshots;
DROP POLICY IF EXISTS "Users can delete own analysis snapshots" ON ai_pair_analysis_snapshots;

DROP POLICY IF EXISTS "Users can view own prediction accuracy" ON ai_prediction_accuracy;
DROP POLICY IF EXISTS "Users can insert own prediction accuracy" ON ai_prediction_accuracy;
DROP POLICY IF EXISTS "Users can update own prediction accuracy" ON ai_prediction_accuracy;
...
SELECT
  table_name,
  CASE
    WHEN table_name IN ('ai_pair_predictions', 'ai_pair_analysis_snapshots', 'ai_prediction_accuracy')
    THEN '✓ Created successfully'
    ELSE ''
  END as status
FROM information_schema.tables
WHERE table_schema = 'public'
  AND table_name IN ('ai_pair_predictions', 'ai_pair_analysis_snapshots', 'ai_prediction_accuracy')
ORDER BY table_name;
