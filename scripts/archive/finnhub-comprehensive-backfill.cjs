...
/**
 * Finnhub Comprehensive Backfill Script
 *
 * Backfills historical candle data from Finnhub for all major pairs and timeframes.
 * This provides native OHLC data with proper high/low wicks for accurate charting.
 *
 * Timeframes covered:
 * - M15: ~7 days
 * - M30: ~14 days
 * - H1: ~30 days
 * - H4: ~50 days
 * - D1: ~10 months (300 days)
 * - W1: ~3.8 years (200 weeks)
 */
...
const NETLIFY_FUNCTION_URL = process.env.VITE_NETLIFY_SITE_URL
  ? `${process.env.VITE_NETLIFY_SITE_URL}/.netlify/functions/finnhub-import`
  : 'http://localhost:8888/.netlify/functions/finnhub-import';
...
// Rate limiting
const RATE_LIMIT_DELAY_MS = 1200; // Finnhub free tier: 60 calls/minute = 1 call per second
const BATCH_DELAY_MS = 2000; // Extra delay between major batches
...
async function main() {
  console.log('\n');
  console.log('╔════════════════════════════════════════════════════════════════════════════╗');
  console.log('║                  FINNHUB COMPREHENSIVE BACKFILL                            ║');
  console.log('║                  Native OHLC Data with Full Wicks                          ║');
  console.log('╚════════════════════════════════════════════════════════════════════════════╝');
  console.log('\n');
  ...
  if (successful.length > 0) {
    ...
    successful.forEach(r => {
      ...
      console.log(`  ✅ ${r.symbol} ${r.timeframe}: ${inserted} candles`);
    });
    console.log('\n');
  }
