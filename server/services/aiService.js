...
Return a JSON object with this exact structure:
{
  ...
  getMockAnalysis(selectedPair = 'EURUSD', specifiedRiskLevel = null) {
    ...
    const basePrice = basePrices[selectedPair] || basePrices['EURUSD'];
    const isGold = selectedPair === 'XAUUSD';
    const priceStep = isGold ? 0.10 : 0.0001;
...
Generate a professional journal entry for this trading event: ${eventType}
Trade data: ${JSON.stringify(tradeData)}

Return a JSON object with:
{
  "title": "Entry title",
  "content": "Detailed analysis referencing applicable Pipnosis Laws",
  "confidence_level": "high|medium|low"
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          { role: 'system', content: systemPrompt },
          { role: 'user', content: `Event: ${eventType}, Data: ${JSON.stringify(tradeData)}` }
        ],
        temperature: 0.7,
        max_tokens: 500
      });
  ...
  getMockJournalEntry(eventType, tradeData) {
    const entries = {
      ...
      trade_exit: {
        ...
        content: 'Position closed according to predetermined exit strategy. Law #7 (Cut Losses Early, Let Winners Run) guided the exit decision. Risk management protocols followed.',
        confidence_level: 'high'
      }
    };
