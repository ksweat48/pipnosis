            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {subscriptionPackages.map(pkg => (
                  <div key={pkg.id} className="relative group">
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl opacity-20 group-hover:opacity-40 transition duration-300 blur" />

                    <div className="relative bg-gradient-to-br from-blue-900/40 to-cyan-900/40 backdrop-blur-xl rounded-xl p-6 border-2 border-blue-500/30 hover:border-blue-500/60 transition-all shadow-2xl">
                      {pkg.badge && (
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-full text-white text-xs font-bold shadow-lg flex items-center gap-1">
                          <Sparkles className="w-3 h-3" />
                          {pkg.badge}
                      ...
                      <div className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-400 mb-1">${pkg.priceUsd.toFixed(2)}</div>
                      <div className="text-gray-400 text-sm mb-4">/month</div>
                      <button
                        onClick={() => handlePurchaseClick(pkg)}
                        disabled={processingPayment === pkg.id}
                        className="w-full px-4 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all shadow-lg hover:shadow-blue-500/25 hover:scale-105 active:scale-95 disabled:scale-100"
                      >
