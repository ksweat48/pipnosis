...
import { CheckCircle2, Activity, AlertTriangle, Clock } from 'lucide-react';
...
export function TradeWellnessIndicator({ tradeId, compact = false, showTooltip = true }: TradeWellnessIndicatorProps) {
  ...
  return (
    <div className="space-y-1">
      <div
        className={`flex items-center gap-2 px-3 py-2 rounded-lg ${colorClass} cursor-pointer`}
        onClick={() => setShowDetails(!showDetails)}
      >
        <div className="flex items-center gap-1.5">
          {getStatusIcon(wellness.latest_status)}
          <span className="text-xs font-medium">
            Alpha: {wellness.latest_status}
          </span>
        </div>
      ...
      {showDetails && wellness.latest_note && (
        <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-3 text-xs">
          ...
          <div className="mt-2 pt-2 border-t border-gray-700 flex items-center justify-between text-[10px] text-gray-500">
            <span>{wellness.total_checks} total checks</span>
            {wellness.concerning_checks > 0 && (
              <span className="text-orange-400">
                {wellness.concerning_checks} concerning
              </span>
            )}
          </div>
        </div>
