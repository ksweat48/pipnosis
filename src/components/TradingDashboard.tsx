              ...
              <div key={trade.id} className="p-4 border-b border-slate-700 last:border-b-0">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <span className="text-slate-300 ml-1 font-mono">{trade.lotSize}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Entry:</span>
                    <span className="text-slate-300 ml-1 font-mono">{trade.entry.toFixed(5)}</span>
                  </div>
                  <div>
                    <span className="text-slate-400">Current:</span>
                    <span className="text-white ml-1 font-mono">{trade.current.toFixed(5)}</span>
                  </div>
              ...
              <tbody>
                {openTrades.map((trade) => (
                  <tr key={trade.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                    ...
                    <td className="p-4 text-slate-300 font-mono">{trade.lotSize}</td>
                    <td className="p-4 text-slate-300 font-mono">{trade.entry.toFixed(5)}</td>
                    <td className="p-4 text-white font-mono">{trade.current.toFixed(5)}</td>
                    <td className="p-4">
                      <span className={`font-semibold ${trade.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        {trade.pnl >= 0 ? '+' : ''}${trade.pnl.toFixed(2)}
                      </span>
