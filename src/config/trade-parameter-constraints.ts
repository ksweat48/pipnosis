...
export function validateStopLossDistance(
  ...
  if (actualDistancePips > 0 && actualDistancePips < constraint.minPips) {
    violations.push(
      `Distance ${actualDistancePips.toFixed(1)} pips is below minimum ${constraint.minPips} pips for ${symbol}`
    );
  }
