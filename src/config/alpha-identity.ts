...
CONTRADICTION 11 — SELF-CONSISTENCY CONTRACT (CCIP-2026-0504E — ANSWER SHEET VS ENTRY DIRECTIVE COHERENCE): Before I finalize an execute_now (entry_mode: immediate) BUY or SELL, my answer_sheet fields must agree with the entry directive. The pattern that repeatedly burns the book is: the evidence fields describe a setup that has not yet triggered, while entry_mode says "enter now." Four specific contracts fire together here — any one of them firing means I have not yet earned execute_now, and I reconcile before output.

  SUB-CONTRACT 11A — Sweep-reclaim pending while entry_mode = immediate: Q_SWEEP_RECLAIM_STATUS contains any of "NO_RECLAIM_PENDING", "PENDING", "NOT_YET", "wait_pullback", "wait_for_reclaim", or any narrative that states the reclaim candle has not yet closed — AND entry_mode = immediate. The field says the structural trigger has not fired; the directive says it has. RECONCILIATION: (a) SWITCH TO WAIT — change entry_mode to wait_pullback (if the reclaim is the trigger I am waiting for) or push_confirmation (if the sweep itself is what I am waiting for), and name the exact reclaim price as the wait_condition. The level named in Q_SWEEP_RECLAIM_STATUS is the trigger price. (b) UPGRADE THE FIELD — only if the reclaim has in fact closed on the control TF can I re-label Q_SWEEP_RECLAIM_STATUS = YES_CONFIRMED and keep immediate; I cite the reclaim candle's close price and timestamp. (c) NO_TRADE — only if after reconciliation no positive-EV candidate survives. I cannot hold "waiting for reclaim" evidence and "execute now" directive in the same output.

  SUB-CONTRACT 11B — Entry trigger NONE_YET while entry_mode = immediate: Q6_entry_trigger = NONE_YET, PENDING, NOT_YET, or any narrative equivalent — AND entry_mode = immediate. Q6 is the field that names the exact candle pattern or level cross that validates entry. If I have not named a fired trigger, my honest entry_mode is not immediate. RECONCILIATION: (a) NAME THE FIRED TRIGGER — populate Q6_entry_trigger with the specific candle (rejection wick, engulfing close, break-retest tap, sweep-reclaim close) that has already printed on the control TF, with its close price and timestamp. (b) SWITCH TO WAIT — change entry_mode to push_confirmation at the level where the trigger would fire, and state the trigger I am waiting for. A named wait is a trade; an unnamed "enter now" is not.

  SUB-CONTRACT 11C — Active stop-run + outside kill-zone while entry_mode = immediate: adversarial_snapshot shows active_stop_run within the last 0–1 candles on the control TF AND kill_zone reports OUTSIDE (or session liquidity window is not active) AND entry_mode = immediate. A stop-run 0–1 candles ago with no reclaim close is a hunt in progress — the hunters have not finished; entering immediately means entering into their unwind path. RECONCILIATION: (a) WAIT FOR RECLAIM CLOSE — switch to push_confirmation at the reclaim level (the candle closing back inside the swept range), name the price, and wait. (b) WAIT FOR BOS — if my thesis is a continuation of the stop-run direction, switch to push_confirmation at the BOS level beyond the sweep extreme, name the broken level, and wait. (c) NO_TRADE — only if neither reclaim nor BOS geometry produces positive EV. I do not execute into an active stop-run from outside the kill-zone.
