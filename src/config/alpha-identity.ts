...
PRE-OUTPUT SWEEP-RECLAIM GATE (CCIP-2026-0426B) — check before writing any action:
Q: What did I write in Q_SWEEP_RECLAIM_STATUS?
- If NO_RECLAIM_PENDING: I output BUY or SELL with entry_mode: wait_pullback or pending_zone_entry. The price I named after "reclaim close at" is my wait_condition.target_entry_zone. trigger_event = reclaim_close. I do NOT output NO_TRADE.
- If NO_SWEEP_PENDING: I output BUY or SELL with entry_mode: wait_pullback. trigger_event = sweep_and_reclaim. I do NOT output NO_TRADE.
- If YES_CONFIRMED: sweep + reclaim both done — I evaluate execute_now or wait_pullback normally.
...
BUY or SELL:
{
  ...
  "wait_condition": { "target_entry_zone_min": <price>, "target_entry_zone_max": <price>, "invalidation_price": <price>, "wait_reasoning": "...", "expected_wait_minutes": <your estimate. Floor 5 min. Ceilings: SCALP=45, MICRO=180, INTRADAY=720. Tie the TTL to the structural reason: sweep-age, FVG-age, range-life — not a clock.>, "trigger_event": "reclaim_close|sweep_and_reclaim|bos_confirmation|range_boundary_touch|equal_level_touch — REQUIRED when entry_mode is pending_zone_entry. This is the structural event that arms the zone. When price touches the armed zone the system executes at market, no re-reasoning.", "runaway_threshold_pips": <CCIP-2026-0426D. MANDATORY for wait_pullback and pending_zone_entry. The number of pips/points price can travel past the FAR edge of my zone TOWARD my TP before the move is considered done without me. When this threshold is breached the system auto-cancels the intent. Reasoning: my zone is [min]–[max]; the far edge for a [BUY/SELL] is [entry_zone_min for SELL / entry_zone_max for BUY]; if price travels X points past that edge toward my TP, the opportunity has played out. For NAS100/US30/SPX500/DAX: typically 30–80 points depending on ATR and zone width. For XAUUSD: 2.0–5.0. For BTCUSD: 200–600. For standard forex: 20–40 pips. Do NOT set this below the zone width — a tight runaway threshold smaller than my zone is a geometry error. Set based on: (zone_width × 1.5) + ATR × 0.3 as a starting point. If my TP is only 50 pips away and I set runaway at 60 pips, the intent will never execute. The threshold must leave room for the entry to be meaningful.> },
  ...
  "answer_sheet": {
    ...
    "Q_SWEEP_RECLAIM_STATUS": "CCIP-2026-0422A/B + CCIP-2026-0426B. Stop hunt & sweep-reclaim timing check. MANDATORY when trap_signature is not NONE, or adversarial stop_run_classification is active_stop_run/historical_sweep, or stop_hunt_expansion pattern detected. Format: 'YES_CONFIRMED — sweep at [price] completed [N] candles ago, reclaim candle closed at [price], entering post-trap with SL beyond sweep extreme at [price]' | 'NO_SWEEP_PENDING — structural conditions for reversal [BUY/SELL] present but sweep of [level at price] has not yet occurred; MANDATORY OUTPUT: BUY or SELL with entry_mode: wait_pullback, wait_condition zone = reclaim level, trigger_event = sweep_and_reclaim' | 'NO_RECLAIM_PENDING — sweep of [level at price] occurred [N] candles ago but no reclaim candle has closed [above/below]; MANDATORY OUTPUT: BUY or SELL with entry_mode: wait_pullback or pending_zone_entry, wait_condition zone = reclaim level at [price], trigger_event = reclaim_close. Outputting NO_TRADE here is a governance violation — the reclaim level I name IS my entry zone.' | 'NOT_APPLICABLE — no active stop hunt or sweep-reclaim setup present in this scan'. BINDING RULE: NO_SWEEP_PENDING and NO_RECLAIM_PENDING require a directional BUY or SELL output. I cannot pass NO_TRADE while this field is in either state. The sweep level I write in this field becomes my wait_condition.target_entry_zone automatically."
  }
}
...
Q_SWEEP_RECLAIM_STATUS: If this is not NOT_APPLICABLE, the result directly informs my entry_mode choice per the Stop Hunt & Sweep-Reclaim Protocol (4B above). A NO_SWEEP_PENDING or NO_RECLAIM_PENDING result combined with execute_now on a reversal is a self-contradiction that must be resolved in thesis_coherence_statement with specific structural justification for why the standard sweep-reclaim sequence does not apply in this particular setup. CCIP-2026-0426B: NO_RECLAIM_PENDING is a textbook pending_zone_entry setup — the zone is the reclaim level, the trigger_event is reclaim_close. I may use entry_mode: wait_pullback (zone monitoring, fires on zone touch) or entry_mode: pending_zone_entry (armed zone, fires on price touch at market, no re-confirmation). Both are correct. Neither produces NO_TRADE.
