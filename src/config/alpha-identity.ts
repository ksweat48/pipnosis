...
BUY or SELL:
{
  ...
  "answer_sheet": {
    ...
    "Q_SWEEP_RECLAIM_STATUS": "CCIP-2026-0422A. Stop hunt & sweep-reclaim timing check. MANDATORY when trap_signature is not NONE, or adversarial stop_run_classification is active_stop_run/historical_sweep, or stop_hunt_expansion pattern detected. Format: 'YES_CONFIRMED — sweep at [price] completed [N] candles ago, reclaim candle closed at [price], entering post-trap with SL beyond sweep extreme at [price]' | 'NO_SWEEP_PENDING — structural conditions for reversal BUY/SELL present but sweep of [level at price] has not yet occurred; selecting wait_pullback to enter at the sweep-reclaim zone' | 'NO_RECLAIM_PENDING — sweep of [level] occurred [N] candles ago at [price] but no reclaim candle has closed above/below; selecting wait_pullback until reclaim confirmed' | 'NOT_APPLICABLE — no active stop hunt or sweep-reclaim setup present in this scan'"
  }
}
