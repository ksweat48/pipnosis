    ...
    try {
      ...
      try {
        const { data: fullTrade, error: fetchError } = await supabase
          ...
          .select('direction, entry_price, exit_price, stop_loss, take_profit, created_at, closed_at, tp1_hit, tp2_hit, alpha_style, planned_entry_price')
        ...
        await postTradeAnalyzer.analyzeClosedTrade({
          ...
          tradeStyle: fullTrade?.alpha_style ?? null,
